...
def build_balance_data(args):
    ...
    # Index all columns by label for convenient lookup
    dict_name = {}
    dict_paths = {}
    dict_durations = {}
    for idx, label in enumerate(labels):
        ...
        if abs(durations[idx] - 9) < 3:    # Filter durations to avoid overly long utterances and keep near the average
            dict_paths[label].append(paths[idx])
            dict_durations[label].append(durations[idx])


    # Randomly sample speakers (args.num_spk distinct labels) into random_num_spk
    candi_spk = []
    for label in range(max(labels) + 1):
        if args.utt_per_spk <= len(dict_paths[label]):  # Filter candidates to ensure enough utterances
    ...
    for label in random_num_spk:            #dict_name[label]  dict_paths[label]  label  dict_durations[label]
        # For each sampled speaker (label), randomly sample utt_per_spk utterance indices into random_utt_per_spk
