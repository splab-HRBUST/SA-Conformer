...
class FrequencyBandModulation(nn.Module):
    def __init__(self, 
                ...
                spatial_kernel=3,
                init='zero',
                max_size=(64, 64), # max size for precomputed masks
                **kwargs,
                ):
        super().__init__()
        self.k_list = k_list
        ...
        # Build attention conv layers (logic unchanged)
        if spatial == 'conv':
        ...
        # Core optimization: precompute and cache masks for different frequency bands
    ...
    def _precompute_masks(self, max_size, k_list):
        """
        Precompute a set of masks at the maximum resolution during initialization.
        """
        ...
        # Stack masks into a tensor: (num_masks, max_h, max_w//2 + 1)
    ...
    def forward(self, x, att_feat=None):
        ...
        # Core optimization: resize cached masks
        # Interpolate cached masks to the current feature-map frequency size
        # Note the frequency-domain size is (h, w//2 + 1)
        freq_h, freq_w = h, w // 2 + 1
        
        # Convert masks from (num_masks, 1, max_h, max_w//2+1) to (num_masks, 1, h, w//2+1)
        # Use nearest interpolation since it is fast and accurate for 0/1 masks
        current_masks = F.interpolate(self.cached_masks.float(), size=(freq_h, freq_w), mode='nearest')

        for idx, freq in enumerate(self.k_list):
            # Fetch mask directly from cache
            mask = current_masks[idx]

            # Apply mask and perform inverse FFT
            # `s=(h,w)` ensures irfft2 output matches the original `x` size
            ...
            # Multiply attention weights with the high-frequency component
