...
def load_audio(filename, second=2):
    ...
    try:
        if ext == ".wav":
            try:
                ...
                waveform, sample_rate = sf.read(filename, dtype="float64", always_2d=False)
        else:
            waveform, sample_rate = sf.read(filename, dtype="float64", always_2d=False)
    except Exception as e:
        raise RuntimeError("Failed to read audio: {}".format(filename)) from e
    ...
    if audio_length <= length:
        ...
        waveform = waveform[start:start + length].astype(np.float64, copy=False)
